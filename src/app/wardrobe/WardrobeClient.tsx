...
const UploadClothing = dynamic(() => import('../../components/UploadClothing'), {
  loading: () => <div className="h-[520px] animate-pulse rounded-[8px] bg-white/7" />
})

const WardrobeList = dynamic(() => import('../../components/WardrobeList'), {
  loading: () => (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
      {Array.from({ length: 8 }).map((_, index) => (
        <div key={index} className="h-64 animate-pulse rounded-[8px] bg-white/7" />
      ))}
    </div>
  )
})
